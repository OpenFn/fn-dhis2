...
class AggregatedEvent
  ...
  def self.push(credentials, message)
    #TODO this should be exploded into describe and prepare as well
    message_as_hash = JSON.parse(message)
    raw_event = message_as_hash["event"]

    associated_tracked_entities = create_tracked_entities(credentials, raw_event)

    associated_tracked_entities.each do |ate_id|
      enroll_tracked_entity(credentials, raw_event, ate_id)

      linked_event = build_event_data(raw_event, ate_id)

      create_event(credentials, linked_event)
    end
  end

  # Class methods aren't really private but if they were
  private

  def self.create_tracked_entities(credentials, raw_event)
    raw_event["instances"].inject([]) do |memo, instance_set|
  ...
  def self.enroll_tracked_entity(credentials, raw_event, ate_id)
    data = {
      "trackedEntityInstance" => ate_id, "orgUnit" => raw_event["orgUnit"],
  ...
  def self.build_event_data(raw_event, ate_id)
    {
      "event" => {
        "program" => raw_event["program"],
  ...
  def self.create_event(credentials, linked_event)
    ...
    puts "New event is at #{response.headers["location"]}"
    response
  end
end
